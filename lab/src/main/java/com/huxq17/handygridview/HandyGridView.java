...
public class HandyGridView extends GridView implements AdapterView.OnItemLongClickListener, AdapterView.OnItemClickListener, ICarrier {
    ...
    private boolean enableCutInLine = true;
    ...
    public void enableCutInLine(boolean enable) {
        enableCutInLine = enable;
        mScrollRunner.enableAnimation(enable);
    }
    ...
    protected void layoutChildren() {
        ...
        if (mDraggedView != null) {
            ...
            dispatchItemReleased(mDraggedPosition);
    ...
    private boolean handleTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mDraggedView != null) {
                    int targetPosition = getTargetPosition(ev);
                    releaseDraggedView(targetPosition);
    ...
    private int getTargetPosition(MotionEvent ev) {
    ...
    private void releaseDraggedView(int targetPosition) {
        ...
        dispatchItemReleased(targetPosition);
        if (mDraggedView.isPressed()) {
            mDraggedView.setPressed(false);
        }
    }

    private void dispatchItemReleased(int targetPosition) {
        View targetView = super.getChildAt(targetPosition - mFirstVisibleFirstItem);
        if (mItemCapturedListener != null && !isFixedPosition(mDraggedPosition)) {
            mItemCapturedListener.onItemReleased(mDraggedView, targetView,
