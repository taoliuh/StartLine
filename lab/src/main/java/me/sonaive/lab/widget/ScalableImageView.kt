...
/**
 * Created by liutao on 2020-03-24.
 *
 * A {@link ImageView} with 2 new custom scale types :  app:scaleType="fitTop" &
 * app:scaleType="fitBottom". Use it if you need something like scaleType="centerCrop|fitTop" or
 * scaleType="centerCrop|fitBottom"
 */
class ImageView @JvmOverloads constructor(
    ...
    init {
        val typedArray = context.theme.obtainStyledAttributes(attrs,
            ...
            0)
        val customScaleType =
            ScaleType.getType(typedArray.getString(R.styleable.ImageView_scaleType))
        scaleType = when (customScaleType) {
            ...
            ScaleType.FIT_TOP -> ImageView.ScaleType.MATRIX.also {
                matrixType = MatrixType.FIT_TOP
            }
            ScaleType.FIT_BOTTOM -> ImageView.ScaleType.MATRIX.also {
                matrixType = MatrixType.FIT_BOTTOM
            }
        }
    }
    ...
    private fun computeDrawableMatrix(matrixType: MatrixType) = drawable?.let {
        ...
        val drawableRect = when (matrixType) {
            MatrixType.FIT_BOTTOM -> RectF(
                0f,
                drawableHeight - offset,
                drawableWidth,
                drawableHeight - TRUNCATE_PATCH
            )
            MatrixType.FIT_TOP -> RectF(0f,
                TRUNCATE_PATCH, drawableWidth, offset)
        }
